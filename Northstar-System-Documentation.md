...
_This document was produced by reading the repository directly (via the GitHub API/codeload, not assumed from the plan) on the date of writing. Where a claim above depends on a specific file, that file is named so it can be re-verified against the current `main` branch._
